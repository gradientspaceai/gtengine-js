// gtengine-js: TypeScript port of Geometric Tools Engine (GTE) MinimumVolumeBox3Rational.h
...
//   MinimumVolumeBox3<T, IndexType, MVB3Rational>
// See MinimumVolumeBox3.ts for the documentation of the algorithm and the
// queries. This is the exact-arithmetic sibling of
// MinimumVolumeBox3FloatingPoint. The entire search over candidate boxes is
// computed with exact arithmetic: the mesh geometry, the hill climbing that
// locates the extreme hull vertices, the level-curve sampling and the axes
// use BSNumber and the candidate volumes use BSRational. The axes are never
// normalized, since normalization is not an exact operation; the volume of a
// candidate is therefore divided by |axis[2]|^2 (the axis[0] and axis[1] of a
// candidate are perpendicular and axis[2] is their cross product, so
// |axis[0]| * |axis[1]| * |axis[2]| = |axis[2]|^2). The results are exact but
// the computations are much slower than those of the floating-point pipeline.
//
// Upstream selects the precision of the exact types with a fixed number of
// 32-bit words (2561 words for T = double). The port's BSNumber and
// BSRational are backed by bigint, so they grow as needed and no such bound
// is required.
...
export interface MinimumVolumeBox3RationalEdge {
    v: [number, number];
    t: [number, number];
}
...
export interface MinimumVolumeBox3RationalTriangle {
...
// itself.
export interface MinimumVolumeBox3RationalCandidate {
    ...
    edge: [MinimumVolumeBox3RationalEdge, MinimumVolumeBox3RationalEdge];
    N: [NVector3, NVector3];
    M: [NVector3, NVector3];
    f00: BSNumber;
    f10: BSNumber;
    ...
    // The axes are not normalized; see the file comments.
    axis: [NVector3, NVector3, NVector3];
...
export interface MinimumVolumeBox3RationalResult {
    dimension: number;
    box: OrientedBox3;
    volume: number;
}
...
function nZero(): BSNumber {
    return new BSNumber();
}

function nOne(): BSNumber {
    return BSNumber.fromNumber(1);
}
...
function nUnit3(d: number): NVector3 {
    const u = nZero3();
    u[d] = BSNumber.fromNumber(1);
    return u;
}

function nClone(u: NVector3): NVector3 {
    return [u[0].clone(), u[1].clone(), u[2].clone()];
}

function nNegate3(u: NVector3): NVector3 {
    return [u[0].negated(), u[1].negated(), u[2].negated()];
}
...
function nDot3(u: NVector3, v: NVector3): BSNumber {
    return u[0].mul(v[0]).add(u[1].mul(v[1])).add(u[2].mul(v[2]));
}

// The port of 's * u + t * v' for exact 3-tuples.
function nAxpby3(s: BSNumber, u: NVector3, t: BSNumber, v: NVector3): NVector3 {
    return [
        s.mul(u[0]).add(t.mul(v[0])),
        s.mul(u[1]).add(t.mul(v[1])),
        s.mul(u[2]).add(t.mul(v[2]))
    ];
}
...
function makeEdge(): MinimumVolumeBox3RationalEdge {
    return { v: [invalidIndex, invalidIndex], t: [invalidIndex, invalidIndex] };
}

function makeTriangle(): MinimumVolumeBox3RationalTriangle {
    return {
...
function makeCandidate(): MinimumVolumeBox3RationalCandidate {
    return {
        ...
        axis: [nUnit3(0), nUnit3(1), nUnit3(2)],
        minSupportIndex: [invalidIndex, invalidIndex, invalidIndex],
        maxSupportIndex: [invalidIndex, invalidIndex, invalidIndex],
        volume: new BSRational()
    };
}

// The port of C++ candidate assignment (value semantics). The vectors are
// cloned; the edges are shared because they are immutable.
function assignCandidate(dst: MinimumVolumeBox3RationalCandidate,
    src: MinimumVolumeBox3RationalCandidate): void {
    dst.edgeIndex = [src.edgeIndex[0], src.edgeIndex[1]];
    dst.edge = [src.edge[0], src.edge[1]];
    dst.N = [nClone(src.N[0]), nClone(src.N[1])];
    dst.M = [nClone(src.M[0]), nClone(src.M[1])];
    dst.f00 = src.f00.clone();
    dst.f10 = src.f10.clone();
    dst.f01 = src.f01.clone();
    dst.f11 = src.f11.clone();
    dst.levelCurveProcessorIndex = src.levelCurveProcessorIndex;
    dst.axis = [nClone(src.axis[0]), nClone(src.axis[1]), nClone(src.axis[2])];
    ...
    dst.volume = src.volume.clone();
}

function cloneCandidate(src: MinimumVolumeBox3RationalCandidate):
    MinimumVolumeBox3RationalCandidate {
...
type LevelCurveProcessor = (this: MinimumVolumeBox3Rational,
    c: MinimumVolumeBox3RationalCandidate,
    mvc: MinimumVolumeBox3RationalCandidate) => void;

export class MinimumVolumeBox3Rational {
    ...
    // information.
    protected mEdges: MinimumVolumeBox3RationalEdge[];
    protected mEdgeIndices: Array<[number, number]>;
    protected mTriangles: MinimumVolumeBox3RationalTriangle[];
    ...
    protected mAlignedCandidate: MinimumVolumeBox3RationalCandidate;

    // The information for the minimum-volume bounding box of the vertices.
    protected mMinimumVolumeObject: MinimumVolumeBox3RationalCandidate;
    ...
    // Construction. To execute in the main thread, set numThreads to 0 or 1.
    // The port ignores values of 2 or larger (it is single-threaded) but
    // accepts them so that calling code is portable.
    ...
    compute(points: readonly Vector[], lgMaxSample: number):
        MinimumVolumeBox3RationalResult {
    ...
    protected computeConvexHull(points: readonly Vector[]):
        ...
        if (dimension === 0) {
            ...
            // Upstream bug (MinimumVolumeBox3Rational.h,
    ...
    protected createMeshTopology(numTriangles: number, indices: readonly number[],
        ...
        this.mEdges = new Array<MinimumVolumeBox3RationalEdge>(numE);
        for (let i = 0; i < numE; ++i) {
            this.mEdges[i] = makeEdge();
        }
        this.mEdgeIndices = [];
        this.mTriangles = new Array<MinimumVolumeBox3RationalTriangle>(numT);
    ...
    protected extractMeshGeometry(vertices: readonly Vector[]): void {
        ...
        logAssert(numVertices === this.mNVertices.length,
        ...
        // Create the triangles and normals to the triangles. The normals are
        // not normalized to avoid floating-point rounding errors. This is
        // necessary for creating the vertex adjacency data structure that
        // supports the hill-climbing algorithm that computes extreme hull
        // points in a specified direction. The hill climbing is implemented
        // in getExtreme.
    ...
    protected computeAlignedCandidate(): void {
        ...
        for (let j = 0; j < 3; ++j) {
            ...
            pmax[j] = emax.dMax;
            const emin = this.getExtreme(nNegate3(axis));
            this.mAlignedCandidate.minSupportIndex[j] = emin.vMax;
            pmin[j] = emin.dMax.negated();
        }
        const diff = nSub3(pmax, pmin);
        this.mAlignedCandidate.volume = BSRational.fromBSNumber(
            diff[0].mul(diff[1]).mul(diff[2]));
    }
    ...
    protected getExtreme(direction: NVector3): { vMax: number, dMax: BSNumber } {
        let vMax = this.mVClimbStart;
        let dMax = nDot3(direction, this.mNVertices[vMax]);
        ...
        for (let i = 0; i < this.mNVertices.length; ++i) {
            ...
            for (let j = 1; j <= numAdjacent; ++j) {
                ...
                const dCandidate = nDot3(direction, this.mNVertices[vCandidate]);
                if (dCandidate.greaterThan(dLocalMax)) {
    ...
    protected computeVolume(candidate: MinimumVolumeBox3RationalCandidate): void {
        ...
        // now.
        candidate.axis[2] = nCross3(candidate.axis[0], candidate.axis[1]);

        const pmin = nZero3(), pmax = nZero3();
        candidate.minSupportIndex[0] = this.mEdges[candidate.edgeIndex[0]].v[0];
        pmin[0] = nDot3(candidate.axis[0], this.mNVertices[candidate.minSupportIndex[0]]);
        ...
        pmin[1] = nDot3(candidate.axis[1], this.mNVertices[candidate.minSupportIndex[1]]);
        ...
        // computation above.
        const e2min = this.getExtreme(nNegate3(candidate.axis[2]));
        candidate.minSupportIndex[2] = e2min.vMax;
        pmin[2] = e2min.dMax.negated();
        const e2max = this.getExtreme(candidate.axis[2]);
        candidate.maxSupportIndex[2] = e2max.vMax;
        pmax[2] = e2max.dMax;
        const diff = nSub3(pmax, pmin);
        // The axes are not unit length, so the volume of the box measured in
        // the axis directions must be divided by |axis[0]|*|axis[1]|*|axis[2]|
        // = Dot(axis[2],axis[2]).
        candidate.volume =
            BSRational.fromBSNumber(diff[0].mul(diff[1]).mul(diff[2])).div(
                BSRational.fromBSNumber(nDot3(candidate.axis[2], candidate.axis[2])));
    }

    protected processEdgePair(edgeIndex: readonly [number, number],
        mvCandidate: MinimumVolumeBox3RationalCandidate): void {
        ...
        candidate.N[0] = this.mNNormals[edge0.t[0]];
        candidate.N[1] = this.mNNormals[edge0.t[1]];
        candidate.M[0] = this.mNNormals[edge1.t[0]];
        candidate.M[1] = this.mNNormals[edge1.t[1]];
        candidate.f00 = nDot3(candidate.N[0], candidate.M[0]);
        candidate.f10 = nDot3(candidate.N[1], candidate.M[0]);
        candidate.f01 = nDot3(candidate.N[0], candidate.M[1]);
        candidate.f11 = nDot3(candidate.N[1], candidate.M[1]);

        const s00 = candidate.f00.getSign();
        const s10 = candidate.f10.getSign();
        const s01 = candidate.f01.getSign();
        const s11 = candidate.f11.getSign();
        const bits00 = (s00 > 0 ? 1 : (s00 < 0 ? 2 : 0));
        const bits10 = (s10 > 0 ? 1 : (s10 < 0 ? 2 : 0));
        const bits01 = (s01 > 0 ? 1 : (s01 < 0 ? 2 : 0));
        const bits11 = (s11 > 0 ? 1 : (s11 < 0 ? 2 : 0));
    ...
    protected getMinimumVolumeBox(): { box: OrientedBox3, volume: number } {
        ...
        for (let i = 0; i < 3; ++i) {
            ...
            for (let j = 0; j < 3; ++j) {
                rAxis[i][j] = BSRational.fromBSNumber(mvc.axis[i][j]);
            }
            rSqrLengthAxis[i] = rDot3(rAxis[i], rAxis[i]);

            // The exact pipeline computes the support projections with
            // BSNumber arithmetic and converts them to BSRational, which is
            // exact.
            rPMin[i] = BSRational.fromBSNumber(
                nDot3(mvc.axis[i], this.mNVertices[mvc.minSupportIndex[i]]));
            rPMax[i] = BSRational.fromBSNumber(
                nDot3(mvc.axis[i], this.mNVertices[mvc.maxSupportIndex[i]]));
        }
    ...
    protected minimizerConstantS(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        const half = BSNumber.fromNumber(0.5);
        const t = new Array<BSNumber>(this.mMaxSample + 1);
        t[0] = nZero();
        t[this.mMaxSample] = nOne();
        for (const item of this.mDomainIndex) {
            t[item[0]] = half.mul(t[item[1]].add(t[item[2]]));
        }

        for (let i = 0, j = this.mMaxSample; i <= this.mMaxSample; ++i, --j) {
            c.axis[1] = nAxpby3(t[j], c.M[0], t[i], c.M[1]);
            this.computeVolume(c);
            if (c.volume.lessThan(mvc.volume)) {
                assignCandidate(mvc, c);
            }
        }
    }

    protected minimizerConstantT(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        const half = BSNumber.fromNumber(0.5);
        const s = new Array<BSNumber>(this.mMaxSample + 1);
        s[0] = nZero();
        s[this.mMaxSample] = nOne();
        for (const item of this.mDomainIndex) {
            s[item[0]] = half.mul(s[item[1]].add(s[item[2]]));
        }

        for (let i = 0, j = this.mMaxSample; i <= this.mMaxSample; ++i, --j) {
            c.axis[0] = nAxpby3(s[j], c.N[0], s[i], c.N[1]);
            this.computeVolume(c);
            if (c.volume.lessThan(mvc.volume)) {
                assignCandidate(mvc, c);
            }
    ...
    protected minimizerVariableS(sminNumer: BSNumber, smaxNumer: BSNumber, sDenom: BSNumber,
        c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        const half = BSNumber.fromNumber(0.5);
        const s = new Array<BSNumber>(this.mMaxSample + 1);
        const oms = new Array<BSNumber>(this.mMaxSample + 1);
        s[0] = sminNumer;
        oms[0] = sDenom.sub(sminNumer);
        s[this.mMaxSample] = smaxNumer;
        oms[this.mMaxSample] = sDenom.sub(smaxNumer);
        for (const item of this.mDomainIndex) {
            s[item[0]] = half.mul(s[item[1]].add(s[item[2]]));
            oms[item[0]] = half.mul(oms[item[1]].add(oms[item[2]]));
        }

        for (let i = 0; i <= this.mMaxSample; ++i) {
            c.axis[0] = nAxpby3(oms[i], c.N[0], s[i], c.N[1]);

            const q0 = oms[i].mul(c.f00).add(s[i].mul(c.f10));
            const q1 = oms[i].mul(c.f01).add(s[i].mul(c.f11));
            if (q0.greaterThan(q1)) {
                c.axis[1] = nAxpby3(q0, c.M[1], q1.negated(), c.M[0]);
            } else {
                c.axis[1] = nAxpby3(q1, c.M[0], q0.negated(), c.M[1]);
            }
    ...
    // Upstream bug (MinimumVolumeBox3Rational.h, MinimizerVariableT): the
    // three numerator/denominator parameters are declared 'T const&' rather
    // than 'Number const&' as they are in the sibling MinimizerVariableS.
    // Every caller passes Number (BSNumber) expressions such as -c.f01 and
    // c.f11 - c.f01, so the exact values are silently rounded to the
    // floating-point type T on the way in and then converted back to Number,
    // which defeats the purpose of the exact pipeline. The port declares the
    // parameters with the exact type.
    protected minimizerVariableT(tminNumer: BSNumber, tmaxNumer: BSNumber, tDenom: BSNumber,
        c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        const half = BSNumber.fromNumber(0.5);
        const t = new Array<BSNumber>(this.mMaxSample + 1);
        const omt = new Array<BSNumber>(this.mMaxSample + 1);
        t[0] = tminNumer;
        omt[0] = tDenom.sub(tminNumer);
        t[this.mMaxSample] = tmaxNumer;
        omt[this.mMaxSample] = tDenom.sub(tmaxNumer);
        for (const item of this.mDomainIndex) {
            t[item[0]] = half.mul(t[item[1]].add(t[item[2]]));
            omt[item[0]] = half.mul(omt[item[1]].add(omt[item[2]]));
        }

        for (let i = 0; i <= this.mMaxSample; ++i) {
            const p0 = omt[i].mul(c.f00).add(t[i].mul(c.f01));
            const p1 = omt[i].mul(c.f10).add(t[i].mul(c.f11));
            if (p0.greaterThan(p1)) {
                c.axis[0] = nAxpby3(p0, c.N[1], p1.negated(), c.N[0]);
            } else {
                c.axis[0] = nAxpby3(p1, c.N[0], p0.negated(), c.N[1]);
            }

            c.axis[1] = nAxpby3(omt[i], c.M[0], t[i], c.M[1]);
    ...
    protected pair(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        this.computeVolume(c);
        if (c.volume.lessThan(mvc.volume)) {
            assignCandidate(mvc, c);
    ...
    // vectors are cloned so that the candidate normals cannot be modified.
    private pairOf(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate, i: number, j: number): void {
        c.axis[0] = nClone(c.N[i]);
        c.axis[1] = nClone(c.M[j]);
        this.pair(c, mvc);
    }
    ...
    protected z00Z10Z01Z11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x00
        // 0 0
        // 0 0
    ...
    protected p00Z10Z01Z11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x01
        // 0 0
        // + 0
        ...
        // tmin = 0, tmax = 1, s = 1
        c.axis[0] = nClone(c.N[1]);
        this.minimizerConstantS(c, mvc);

        // smin = 0, smax = 1, t = 1
        c.axis[1] = nClone(c.M[1]);
        this.minimizerConstantT(c, mvc);
    }

    protected m00Z10Z01Z11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x02
        // 0 0
        // - 0
    ...
    protected z00P10Z01Z11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x04
        // 0 0
        // 0 +
    ...
    protected p00P10Z01Z11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x05
        // 0 0
        // + +
    ...
    protected m00P10Z01Z11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x06
        // 0 0
        // - +
        ...
        c.axis[0] = nAxpby3(c.f10, c.N[0], c.f00.negated(), c.N[1]);
        this.minimizerConstantS(c, mvc);
    ...
    protected z00M10Z01Z11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x08
        // 0 0
        // 0 -
    ...
    protected p00M10Z01Z11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x09
        // 0 0
        // + -
        ...
        c.axis[0] = nAxpby3(c.f10.negated(), c.N[0], c.f00, c.N[1]);
        this.minimizerConstantS(c, mvc);
    ...
    protected m00M10Z01Z11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x0a
        // 0 0
        // - -
    ...
    protected z00Z10P01Z11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x10
        // + 0
        // 0 0
        ...
        // smin = 0, smax = 1, t = 0
        c.axis[1] = nClone(c.M[0]);
        this.minimizerConstantT(c, mvc);
    }

    protected p00Z10P01Z11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x11
        // + 0
        // + 0
    ...
    protected m00Z10P01Z11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x12
        // + 0
        // - 0
        ...
        c.axis[1] = nAxpby3(c.f01, c.M[0], c.f00.negated(), c.M[1]);
        this.minimizerConstantT(c, mvc);
    }

    protected z00P10P01Z11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x14
        // + 0
        // 0 +
    ...
    protected p00P10P01Z11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x15
        // + 0
        // + +
    ...
    protected m00P10P01Z11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x16
        // + 0
        // - +
        ...
        // smin = 0
        // smax = -f00 / (f10 - f00), (+)/(+)
        this.minimizerVariableS(nZero(), c.f00.negated(), c.f10.sub(c.f00), c, mvc);

        this.pairOf(c, mvc, 1, 1);
    }

    protected z00M10P01Z11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x18
        // + 0
        // 0 -
        ...
        // smin = 0, smax = 1
        this.minimizerVariableS(nZero(), nOne(), nOne(), c, mvc);
    }

    protected p00M10P01Z11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x19
        // + 0
        // + -
        ...
        // tmin = 0, tmax = 1
        this.minimizerVariableT(nZero(), nOne(), nOne(), c, mvc);
    }

    protected m00M10P01Z11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x1a
        // + 0
        // - -
    ...
    protected z00Z10M01Z11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x20
        // - 0
        // 0 0
    ...
    protected p00Z10M01Z11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x21
        // - 0
        // + 0
        ...
        c.axis[1] = nAxpby3(c.f01.negated(), c.M[0], c.f00, c.M[1]);
        this.minimizerConstantT(c, mvc);
    }

    protected m00Z10M01Z11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x22
        // - 0
        // - 0
    ...
    protected z00P10M01Z11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x24
        // - 0
        // 0 +
    ...
    protected p00P10M01Z11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x25
        // - 0
        // + +
    ...
    protected m00P10M01Z11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x26
        // - 0
        // - +
    ...
    protected z00M10M01Z11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x28
        // - 0
        // 0 -
    ...
    protected p00M10M01Z11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x29
        // - 0
        // + -
        ...
        // smin = 0
        // smax = f00 / (f00 - f10), (+)/(+)
        this.minimizerVariableS(nZero(), c.f00, c.f00.sub(c.f10), c, mvc);

        this.pairOf(c, mvc, 1, 1);
    }

    protected m00M10M01Z11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x2a
        // - 0
        // - -
    ...
    protected z00Z10Z01P11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x40
        // 0 +
        // 0 0
    ...
    protected p00Z10Z01P11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x41
        // 0 +
        // + 0
    ...
    protected m00Z10Z01P11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x42
        // 0 +
        // - 0
    ...
    protected z00P10Z01P11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x44
        // 0 +
        // 0 +
    ...
    protected p00P10Z01P11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x45
        // 0 +
        // + +
    ...
    protected m00P10Z01P11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x46
        // 0 +
        // - +
    ...
    protected z00M10Z01P11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x48
        // 0 +
        // 0 -
        ...
        // 1-t = f11 / (f11 - f10), (+)/(+)
        // M = (1-t) * M0 + t * M1, omit denominator
        c.axis[1] = nAxpby3(c.f11, c.M[0], c.f10.negated(), c.M[1]);
        this.minimizerConstantT(c, mvc);
    }

    protected p00M10Z01P11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x49
        // 0 +
        // + -
        ...
        // smin = f00 / (f00 - f10), (+)/(+)
        // smax = 1
        const f00mf10 = c.f00.sub(c.f10);
        this.minimizerVariableS(c.f00, f00mf10, f00mf10, c, mvc);

        this.pairOf(c, mvc, 0, 1);
    }

    protected m00M10Z01P11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x4a
        // 0 +
        // - -
    ...
    protected z00Z10P01P11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x50
        // + +
        // 0 0
    ...
    protected p00Z10P01P11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x51
        // + +
        // + 0
    ...
    protected m00Z10P01P11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x52
        // + +
        // - 0
    ...
    protected z00P10P01P11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x54
        // + +
        // 0 +
    ...
    protected p00P10P01P11(_c: MinimumVolumeBox3RationalCandidate,
        _mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x55
        // + +
        // + +
        // Nothing to do.
    ...
    protected m00P10P01P11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x56
        // + +
        // - +
    ...
    protected z00M10P01P11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x58
        // + +
        // 0 -
    ...
    protected p00M10P01P11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x59
        // + +
        // + -
    ...
    protected m00M10P01P11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x5a
        // + +
        // - -
    ...
    protected z00Z10M01P11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x60
        // - +
        // 0 0
        ...
        // 1-s = f11 / (f11 - f01), (+)/(+)
        // N = (1-s) * N0 + s * N1, omit denominator
        c.axis[0] = nAxpby3(c.f11, c.N[0], c.f01.negated(), c.N[1]);
        this.minimizerConstantS(c, mvc);
    ...
    protected p00Z10M01P11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x61
        // - +
        // + 0
        ...
        // smin = 0
        // smax = -f01 / (f11 - f01), (+)/(+)
        this.minimizerVariableS(nZero(), c.f01.negated(), c.f11.sub(c.f01), c, mvc);

        this.pairOf(c, mvc, 1, 0);
    }

    protected m00Z10M01P11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x62
        // - +
        // - 0
    ...
    protected z00P10M01P11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x64
        // - +
        // 0 +
    ...
    protected p00P10M01P11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x65
        // - +
        // + +
    ...
    protected m00P10M01P11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x66
        // - +
        // - +
    ...
    protected z00M10M01P11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x68
        // - +
        // 0 -
        ...
        // smin = -f01 / (f11 - f01), (+)/(+)
        // smax = 1
        const f11mf01 = c.f11.sub(c.f01);
        this.minimizerVariableS(c.f01.negated(), f11mf01, f11mf01, c, mvc);

        this.pairOf(c, mvc, 0, 0);
    }

    protected p00M10M01P11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x69
        // - +
        // + -
        ...
        const d = c.f00.mul(c.f11).sub(c.f10.mul(c.f01));
        if (d.getSign() > 0) {
            ...
            // smax = -f01 / (f11 - f01), (+)/(+)
            const f11mf01 = c.f11.sub(c.f01);
            this.minimizerVariableS(nZero(), c.f01.negated(), f11mf01, c, mvc);
        } else if (d.getSign() < 0) {
            // endpoints (0,t0) and (s0,0)
            // smin = 0
            // smax = f00 / (f00 - f10), (+)/(+)
            const f00mf10 = c.f00.sub(c.f10);
            this.minimizerVariableS(nZero(), c.f00, f00mf10, c, mvc);
            ...
            c.axis[0] = nAxpby3(c.f11.sub(c.f10), c.N[0], c.f00.sub(c.f01), c.N[1]);
            this.minimizerConstantS(c, mvc);
            ...
            c.axis[1] = nAxpby3(c.f11.sub(c.f01), c.M[0], c.f00.sub(c.f10), c.M[1]);
            this.minimizerConstantT(c, mvc);
        }
    }

    protected m00M10M01P11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x6a
        // - +
        // - -
    ...
    protected z00Z10Z01M11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x80
        // 0 -
        // 0 0
    ...
    protected p00Z10Z01M11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x81
        // 0 -
        // + 0
    ...
    protected m00Z10Z01M11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x82
        // 0 -
        // - 0
    ...
    protected z00P10Z01M11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x84
        // 0 -
        // 0 +
        ...
        c.axis[1] = nAxpby3(c.f11.negated(), c.M[0], c.f10, c.M[1]);
        this.minimizerConstantT(c, mvc);
    }

    protected p00P10Z01M11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x85
        // 0 -
        // + +
    ...
    protected m00P10Z01M11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x86
        // 0 -
        // - +
        ...
        // smin = -f00 / (f10 - f00), (+)/(+)
        // smax = 1
        const f10mf00 = c.f10.sub(c.f00);
        this.minimizerVariableS(c.f00.negated(), f10mf00, f10mf00, c, mvc);
    }

    protected z00M10Z01M11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x88
        // 0 -
        // 0 -
    ...
    protected p00M10Z01M11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x89
        // 0 -
        // + -
    ...
    protected m00M10Z01M11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x8a
        // 0 -
        // - -
    ...
    protected z00Z10P01M11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x90
        // + -
        // 0 0
        ...
        c.axis[0] = nAxpby3(c.f11.negated(), c.N[0], c.f01, c.N[1]);
        this.minimizerConstantS(c, mvc);
    ...
    protected p00Z10P01M11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x91
        // + -
        // + 0
    ...
    protected m00Z10P01M11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x92
        // + -
        // - 0
        ...
        // smin = 0
        // smax = f01 / (f01 - f11), (+)/(+)
        this.minimizerVariableS(nZero(), c.f01, c.f01.sub(c.f11), c, mvc);

        this.pairOf(c, mvc, 1, 0);
    }

    protected z00P10P01M11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x94
        // + -
        // 0 +
        ...
        // smin = f01 / (f01 - f11), (+)/(+)
        // smax = 1
        const f01mf11 = c.f01.sub(c.f11);
        this.minimizerVariableS(c.f01, f01mf11, f01mf11, c, mvc);

        this.pairOf(c, mvc, 0, 0);
    }

    protected p00P10P01M11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x95
        // + -
        // + +
    ...
    protected m00P10P01M11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x96
        // + -
        // - +
        ...
        if (d.getSign() > 0) {
            ...
            // smax = f01 / (f01 - f11)
            const f01mf11 = c.f01.sub(c.f11);
            this.minimizerVariableS(nZero(), c.f01, f01mf11, c, mvc);
        } else if (d.getSign() < 0) {
            // endpoints (0,t0) and (s0,0)
            // smin = 0
            // smax = -f00 / (f10 - f00), (+)/(+)
            const f10mf00 = c.f10.sub(c.f00);
            this.minimizerVariableS(nZero(), c.f00.negated(), f10mf00, c, mvc);
            ...
            // N = (1-sa) * N0 + sa * N1, omit the denominator
            c.axis[0] = nAxpby3(c.f10.sub(c.f11), c.N[0], c.f01.sub(c.f00), c.N[1]);
            this.minimizerConstantS(c, mvc);
            ...
            // M = (1-ta) * M0 + ta * M1, omit the denominator
            c.axis[1] = nAxpby3(c.f01.sub(c.f11), c.M[0], c.f10.sub(c.f00), c.M[1]);
            this.minimizerConstantT(c, mvc);
        }
    }

    protected z00M10P01M11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x98
        // + -
        // 0 -
    ...
    protected p00M10P01M11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x99
        // + -
        // + -
    ...
    protected m00M10P01M11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0x9a
        // + -
        // - -
    ...
    protected z00Z10M01M11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0xa0
        // - -
        // 0 0
    ...
    protected p00Z10M01M11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0xa1
        // - -
        // + 0
    ...
    protected m00Z10M01M11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0xa2
        // - -
        // - 0
    ...
    protected z00P10M01M11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0xa4
        // - -
        // 0 +
    ...
    protected p00P10M01M11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0xa5
        // - -
        // + +
    ...
    protected m00P10M01M11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0xa6
        // - -
        // - +
    ...
    protected z00M10M01M11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0xa8
        // - -
        // 0 -
    ...
    protected p00M10M01M11(c: MinimumVolumeBox3RationalCandidate,
        mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0xa9
        // - -
        // + -
    ...
    protected m00M10M01M11(_c: MinimumVolumeBox3RationalCandidate,
        _mvc: MinimumVolumeBox3RationalCandidate): void {
        // index = 0xaa
        // - -
        // - -
