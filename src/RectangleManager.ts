// gtengine-js: TypeScript port of Geometric Tools Engine (GTE) RectangleManager.h
// Upstream: David Eberly, Geometric Tools, Redmond WA 98052
// Copyright (c) 1998-2026 David Eberly
// Distributed under the Boost Software License, Version 1.0.
// https://www.boost.org/LICENSE_1_0.txt

// Sort-and-sweep management of a collection of axis-aligned rectangles in 2D.
// The initial set of overlapping pairs is computed by a full sort of the
// rectangle endpoints along each coordinate axis followed by a sweep of the
// x-endpoints. After the rectangles are moved by small amounts, the endpoint
// arrays are nearly sorted, so an insertion sort restores the ordering
// quickly and the overlap set is updated incrementally as endpoints are
// transposed. This is the 2D analogue of BoxManager.
//
// Port notes (mirroring BoxManager.ts, set by B61):
// - Upstream stores 'std::vector<AlignedBox2<Real>>&', a reference to the
//   caller's array. The port stores the array itself, which has reference
//   semantics in TypeScript, so setRectangle() writes through to the caller's
//   array exactly as upstream does. The AlignedBox elements are cloned on the
//   way in and out (setRectangle/getRectangle) because C++ assigns values.
...
interface RectangleEndpoint {
    ...
    // The index of the interval (rectangle) containing this endpoint.
    index: number;
}
...
function endpointLess(e0: RectangleEndpoint, e1: RectangleEndpoint): boolean {
...
function endpointCompare(e0: RectangleEndpoint,
    e1: RectangleEndpoint): number {
...
export class RectangleManager {
    private mRectangles: AlignedBox[];
    private mXEndpoints: RectangleEndpoint[];
    private mYEndpoints: RectangleEndpoint[];
    ...
    // construction.
    constructor(rectangles: AlignedBox[]) {
    ...
    initialize(): void {
        ...
        const intrSize = this.mRectangles.length;
        const endpSize = 2 * intrSize;
        this.mXEndpoints = new Array<RectangleEndpoint>(endpSize);
        this.mYEndpoints = new Array<RectangleEndpoint>(endpSize);
        for (let i = 0, j = 0; i < intrSize; ++i) {
            const rectangle = this.mRectangles[i];

            this.mXEndpoints[j] =
                { type: 0, value: rectangle.min.values[0], index: i };
            this.mYEndpoints[j] =
                { type: 0, value: rectangle.min.values[1], index: i };
            ++j;

            this.mXEndpoints[j] =
                { type: 1, value: rectangle.max.values[0], index: i };
            this.mYEndpoints[j] =
                { type: 1, value: rectangle.max.values[1], index: i };
            ++j;
        }
        ...
        for (let j = 0; j < endpSize; ++j) {
            this.mXLookup[2 * this.mXEndpoints[j].index
                + this.mXEndpoints[j].type] = j;
            this.mYLookup[2 * this.mYEndpoints[j].index
                + this.mYEndpoints[j].type] = j;
        }

        // Active set of rectangles (stored by index in array). Upstream uses
        // a std::set, which iterates in ascending order; the sweep result
        // does not depend on the iteration order, but the order is replicated
        // so the port matches upstream step for step.
        ...
        for (let i = 0; i < endpSize; ++i) {
            ...
            if (endpoint.type === 0) {
                ...
                // interval overlaps with all the active intervals. In 2D we
                // also need to check for y-overlap.
                ...
                for (const activeIndex of sorted) {
                    ...
                    const r0 = this.mRectangles[activeIndex];
                    const r1 = this.mRectangles[index];
                    if (r0.max.values[1] >= r1.min.values[1]
                        && r0.min.values[1] <= r1.max.values[1]) {
    ...
    // because the endpoint values stored internally by this class must also
    // change.
    setRectangle(i: number, rectangle: AlignedBox): void {
        this.mRectangles[i] = rectangle.clone();
        const twoI = 2 * i;
        this.mXEndpoints[this.mXLookup[twoI]].value = rectangle.min.values[0];
        this.mXEndpoints[this.mXLookup[twoI + 1]].value =
            rectangle.max.values[0];
        this.mYEndpoints[this.mYLookup[twoI]].value = rectangle.min.values[1];
        this.mYEndpoints[this.mYLookup[twoI + 1]].value =
            rectangle.max.values[1];
    }

    // Retrieve the current rectangle information. Upstream copies into an
    // output parameter; the port returns a copy.
    getRectangle(i: number): AlignedBox {
        return this.mRectangles[i].clone();
    }
    ...
    // overlapping. The indices are those for the input array. The keys (i,j)
    // are stored so that i < j. The array is sorted lexicographically by
    // (V[0], V[1]), the order in which the upstream std::set iterates.
    getOverlap(): EdgeKey[] {
    ...
    private insertionSort(endpoint: RectangleEndpoint[],
        lookup: number[]): void {
        ...
        const query = new IntrAlignedBox2AlignedBox2TI();
            ...
            while (i >= 0 && endpointLess(key, endpoint[i])) {
                ...
                if (e0.type === 0) {
                    if (e1.type === 1) {
                        ...
                        this.mOverlap.delete(
                            makeOverlapKey(e0.index, e1.index).mapKey());
                    }
                } else {
                    if (e1.type === 0) {
                        ...
                        if (query.test(this.mRectangles[e0.index],
                            this.mRectangles[e1.index]).intersect) {
