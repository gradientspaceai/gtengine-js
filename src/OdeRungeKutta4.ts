// gtengine-js: TypeScript port of Geometric Tools Engine (GTE) OdeRungeKutta4.h
...
export class OdeRungeKutta4 extends OdeSolver<Vector> {
    ...
    update(tIn: number, xIn: Vector): { tOut: number; xOut: Vector } {
        ...
        const fTemp1 = this.mFunction(tIn, xIn);
        let xTemp = add(xIn, mul(halfTDelta, fTemp1));

        // Compute the second step.
        const halfT = tIn + halfTDelta;
        const fTemp2 = this.mFunction(halfT, xTemp);
        xTemp = add(xIn, mul(halfTDelta, fTemp2));

        // Compute the third step.
        const fTemp3 = this.mFunction(halfT, xTemp);
        xTemp = add(xIn, mul(this.mTDelta, fTemp3));

        // Compute the fourth step.
        const sixthTDelta = this.mTDelta / 6;
        const tOut = tIn + this.mTDelta;
        const fTemp4 = this.mFunction(tOut, xTemp);
        const weighted = add(add(fTemp1, mul(2, add(fTemp2, fTemp3))), fTemp4);
        const xOut = add(xIn, mul(sixthTDelta, weighted));
        return { tOut, xOut };
    }
}
