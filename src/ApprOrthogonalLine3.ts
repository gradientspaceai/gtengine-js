// gtengine-js: TypeScript port of Geometric Tools Engine (GTE) ApprOrthogonalLine3.h
...
// (origin,direction). The error for S = (x0,y0,z0) is
// (S-P)^T*(I - D*D^T)*(S-P).
...
export class ApprOrthogonalLine3 extends ApprQuery<Vector> {
    private mParameters: Line;

    // Initialize the model parameters to zero.
    constructor() {
        super();
        this.mParameters = Line.fromOriginDirection(new Vector(3), new Vector(3));
    }
    ...
    fitIndexed(points: readonly Vector[], indices: readonly number[]): boolean {
        if (this.validIndices(points, indices)) {
            ...
            this.mParameters.direction = Vector.fromArray(result.evecs[2]);

            // The fitted line is unique when the maximum eigenvalue has
            // multiplicity 1.
            return result.evals[1] < result.evals[2];
        }
    ...
    error(point: Vector): number {
        ...
        for (let d = 0; d < 3; ++d) {
            diff.values[d] = point.values[d] - this.mParameters.origin.values[d];
        }
    ...
    copyParameters(input: ApprQuery<Vector>): void {
        if (input instanceof ApprOrthogonalLine3) {
