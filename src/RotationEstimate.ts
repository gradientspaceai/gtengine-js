// gtengine-js: TypeScript port of Geometric Tools Engine (GTE) RotationEstimate.h
...
//
// Port notes: the upstream C++ selects the degree with a template parameter
// checked by static_assert. As in the other *Estimate ports (SinEstimate,
// ChebyshevRatioEstimate, ...), the degree is a runtime argument validated by
// logAssert. Upstream's output-reference parameters become return values:
// rotationEstimate returns the 3x3 matrix, rotationDerivativeEstimate returns
...
const C_ROTC0_EST_MAX_ERROR: readonly number[] = [
...
const C_ROTC1_EST_COEFF: readonly (readonly number[])[] = [
    [   // degree 4
...
const C_ROTC1_EST_MAX_ERROR: readonly number[] = [
...
const C_ROTC2_EST_COEFF: readonly (readonly number[])[] = [
    [   // degree 4
...
const C_ROTC2_EST_MAX_ERROR: readonly number[] = [
...
const C_ROTC3_EST_COEFF: readonly (readonly number[])[] = [
    [   // degree 4
...
const C_ROTC3_EST_MAX_ERROR: readonly number[] = [
...
const C_ROTC4_EST_COEFF: readonly (readonly number[])[] = [
    [   // degree 4
...
const C_ROTC4_EST_MAX_ERROR: readonly number[] = [
...
// The degree must be even and in {4,6,8,10,12,14,16}. Upstream's
// 'static_assert((Degree & 1) == 0 && 4 <= Degree && Degree <= 16)'.
function validateDegree(degree: number): void {
    logAssert(degree === Math.trunc(degree) && (degree & 1) === 0
        && 4 <= degree && degree <= 16, 'Invalid degree.');
}

// The shared Horner evaluation of the even polynomial in t.
function evaluate(coeff: readonly (readonly number[])[], t: number,
    degree: number): number {
    validateDegree(degree);

    const c = coeff[(degree - 4) / 2];
    const last = degree / 2;
    const tsqr = t * t;
    let poly = c[last];
    for (let i = 0, index = last - 1; i < last; ++i, --index) {
        poly = c[index] + poly * tsqr;
    }
    return poly;
}
...
// estimate is
//   const t = ...;  // in [0,pi]
//   const result = rotC0Estimate(t, 6);
export function rotC0Estimate(t: number, degree: number): number {
    return evaluate(C_ROTC0_EST_COEFF, t, degree);
}

// Estimate rotc1(t) = (1 - cos(t))/t^2 for t in [0,pi].
export function rotC1Estimate(t: number, degree: number): number {
    return evaluate(C_ROTC1_EST_COEFF, t, degree);
}

// Estimate rotc2(t) = (sin(t) - t*cos(t))/t^3 for t in [0,pi].
export function rotC2Estimate(t: number, degree: number): number {
    return evaluate(C_ROTC2_EST_COEFF, t, degree);
}

// Estimate rotc3(t) = (2*(1-cos(t)) - t*sin(t))/t^4 for t in [0,pi].
export function rotC3Estimate(t: number, degree: number): number {
    return evaluate(C_ROTC3_EST_COEFF, t, degree);
}

// Estimate rotc4(t) = (t - sin(t))/t^3 for t in [0,pi].
export function rotC4Estimate(t: number, degree: number): number {
    return evaluate(C_ROTC4_EST_COEFF, t, degree);
}

export function getRotC0EstimateMaxError(degree: number): number {
    validateDegree(degree);
    return C_ROTC0_EST_MAX_ERROR[(degree - 4) / 2];
}

export function getRotC1EstimateMaxError(degree: number): number {
    validateDegree(degree);
    return C_ROTC1_EST_MAX_ERROR[(degree - 4) / 2];
}

export function getRotC2EstimateMaxError(degree: number): number {
    validateDegree(degree);
    return C_ROTC2_EST_MAX_ERROR[(degree - 4) / 2];
}

export function getRotC3EstimateMaxError(degree: number): number {
    validateDegree(degree);
    return C_ROTC3_EST_MAX_ERROR[(degree - 4) / 2];
}

export function getRotC4EstimateMaxError(degree: number): number {
    validateDegree(degree);
    return C_ROTC4_EST_MAX_ERROR[(degree - 4) / 2];
}

// The skew-symmetric matrix S = {{0,-p2,p1},{p2,0,-p0},{-p1,p0,0}}.
function makeS(p: Vector): Matrix {
...
// The matrix S^2, computed in closed form as upstream does.
function makeSSqr(p: Vector): Matrix {
    const p0 = p.values[0], p1 = p.values[1], p2 = p.values[2];
    const p0p0 = p0 * p0, p0p1 = p0 * p1, p0p2 = p0 * p2;
    const p1p1 = p1 * p1, p1p2 = p1 * p2, p2p2 = p2 * p2;
    return Matrix.fromArray(3, 3, [
        -(p1p1 + p2p2), p0p1, p0p2,
        p0p1, -(p0p0 + p2p2), p1p2,
        p0p2, p1p2, -(p0p0 + p1p1)
...
// The generators of the skew-symmetric matrices, skewE[i] = dS/dp[i].
function makeSkewE(): Matrix[] {
    return [
...
// from a vector (p0,p1,p2) with length t = |(p0,p1,p2)| and skew-symmetric
// matrix S = {{0,-p2,p1},{p2,0,-p0},{-p1,p0,0}}.
export function rotationEstimate(p: Vector, degree: number): Matrix {
    logAssert(p.size === 3, 'RotationEstimate: expecting a 3-tuple.');

    const I = Matrix.identity(3, 3);
    const S = makeS(p);
    const Ssqr = makeSSqr(p);

    const t = length(p);
    const a = rotC0Estimate(t, degree);
    const b = rotC1Estimate(t, degree);
    return addMatrix(addMatrix(I, mulMatrix(a, S) as Matrix),
        mulMatrix(b, Ssqr) as Matrix);
}

// The partial derivatives dR/dp[i] for i in {0,1,2}.
export function rotationDerivativeEstimate(p: Vector,
    degree: number): Matrix[] {
    logAssert(p.size === 3, 'RotationEstimate: expecting a 3-tuple.');

    const skewE = makeSkewE();
    const S = makeS(p);
    const Ssqr = makeSSqr(p);
    ...
    const a = rotC0Estimate(t, degree);
    const b = rotC1Estimate(t, degree);
    const c = rotC2Estimate(t, degree);
    const d = rotC3Estimate(t, degree);
    return computeDerivatives(p, skewE, S, Ssqr, a, b, c, d);
}

// Both the rotation matrix and its partial derivatives, sharing the
// intermediate computations as upstream does.
export function rotationAndDerivativeEstimate(p: Vector, degree: number):
    { R: Matrix, Rder: Matrix[] } {
    logAssert(p.size === 3, 'RotationEstimate: expecting a 3-tuple.');
    ...
    const R = addMatrix(addMatrix(I, mulMatrix(a, S) as Matrix),
        mulMatrix(b, Ssqr) as Matrix);
    const Rder = computeDerivatives(p, skewE, S, Ssqr, a, b, c, d);
    return { R, Rder };
}

// Rder[i] = a*E[i] + b*(S*E[i] + E[i]*S) - p[i]*(c*S + d*S^2)
function computeDerivatives(p: Vector, skewE: readonly Matrix[], S: Matrix,
    Ssqr: Matrix, a: number, b: number, c: number, d: number): Matrix[] {
    const cSpdSsqr = addMatrix(mulMatrix(c, S) as Matrix,
        mulMatrix(d, Ssqr) as Matrix);
    const Rder = new Array<Matrix>(3);
    for (let i = 0; i < 3; ++i) {
        const term0 = mulMatrix(a, skewE[i]) as Matrix;
        const term1 = mulMatrix(b, addMatrix(multiplyAB(S, skewE[i]),
            multiplyAB(skewE[i], S))) as Matrix;
        const term2 = mulMatrix(p.values[i], cSpdSsqr) as Matrix;
        Rder[i] = subMatrix(addMatrix(term0, term1), term2);
    }
    return Rder;
}
