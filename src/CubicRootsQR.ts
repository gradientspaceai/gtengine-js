// gtengine-js: TypeScript port of Geometric Tools Engine (GTE) CubicRootsQR.h
...
// the companion matrix associated with a cubic polynomial.
//
// Port notes: upstream exposes two operator() overloads; the port names them
// solve (from the polynomial coefficients) and solveMatrix (from an upper
...
export type CubicRootsQRMatrix = number[][];

export interface CubicRootsQRResult {
    // The number of Francis QR iterations applied. This equals the
    // requested maxIterations when the matrix did not uncouple (in which
    // case numRoots is 0).
    iterations: number;

    // The number of real roots found (0 to 3). Only the first numRoots
    // elements of 'roots' are valid; the rest are 0.
    numRoots: number;
    roots: number[];
}

export class CubicRootsQR {
    // Solve p(x) = c0 + c1 * x + c2 * x^2 + x^3 = 0.
    solve(maxIterations: number, c0: number, c1: number, c2: number): CubicRootsQRResult {
    ...
    solveMatrix(maxIterations: number, A: CubicRootsQRMatrix): CubicRootsQRResult {
        const roots = [0, 0, 0];

        for (let numIterations = 0; numIterations < maxIterations; ++numIterations) {
            // Apply a Francis QR iteration.
            const tr = A[1][1] + A[2][2];
            const det = A[1][1] * A[2][2] - A[1][2] * A[2][1];
            const X = [
            ...
            if (tr01 + A[1][0] === tr01) {
                roots[0] = A[0][0];
                const numRoots = this.getQuadraticRoots(1, 2, A, roots, 1);
            ...
            if (tr12 + A[2][1] === tr12) {
                roots[0] = A[2][2];
                const numRoots = this.getQuadraticRoots(0, 1, A, roots, 1);
                return { iterations: numIterations, numRoots, roots };
            }
        }
    ...
    private doIteration(V: number[], A: CubicRootsQRMatrix): void {
        const multV = -2 / (V[0] * V[0] + V[1] * V[1] + V[2] * V[2]);
        const MV = [multV * V[0], multV * V[1], multV * V[2]];
        this.rowHouse(0, 2, 0, 2, V, MV, A);
        this.colHouse(0, 2, 0, 2, V, MV, A);

        const Y = [A[1][0], A[2][0]];
        const W = this.house(Y);
        const multW = -2 / (W[0] * W[0] + W[1] * W[1]);
        const MW = [multW * W[0], multW * W[1]];
        this.rowHouse(1, 2, 0, 2, W, MW, A);
        this.colHouse(0, 2, 1, 2, W, MW, A);
    }
    ...
    private rowHouse(rmin: number, rmax: number, cmin: number, cmax: number,
        V: number[], MV: number[], A: CubicRootsQRMatrix): void {
    ...
    private getQuadraticRoots(i0: number, i1: number, A: CubicRootsQRMatrix,
